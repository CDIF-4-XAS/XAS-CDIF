...
def write_units(path: Path) -> int:
    """Emit the units the glossary states for concepts, as a lookup table.

    Not SSSOM: this is not a mapping between vocabularies, it is a fact the
    glossary asserts about a concept. It is a separate artifact because the
    file formats do not record a unit for these quantities, so a reader
    needs somewhere to get one -- and "this concept is dimensionless" is
    true of every file, not of one mapping.
    """
    doc = json.loads(GLOSSARY.read_text(encoding="utf-8"))
    tops = doc["skos:hasTopConcept"]
    concepts = tops + [n for c in tops for n in c.get("skos:narrower", [])]
    rows = []
    for c in concepts:
        unit = c.get("qudt:hasUnit")
        if not unit:
            continue
        if isinstance(unit, dict):
            unit = unit.get("@id", "")
        label = c.get("skos:prefLabel", {})
        rows.append({
            "concept_id": "cdifxas:" + c["@id"].rsplit("/", 1)[-1],
            "concept_label": (label.get("@value", "")
                              if isinstance(label, dict) else str(label)),
            "unit_id": unit,
        })
    rows.sort(key=lambda r: r["concept_id"])
    header = [
        "# Units the CDIF XAS glossary states for concepts.",
        "# Generated by crosswalk/build_crosswalk.py from "
        + GLOSSARY.name + "; edit the glossary, not this file.",
        "# unit_id is a QUDT CURIE; unit: expands to "
    ...
    with path.open("w", encoding="utf-8", newline="") as f:
        ...
        w = csv.DictWriter(
            f, delimiter="\t",
            fieldnames=["concept_id", "concept_label", "unit_id"])
        w.writeheader()
        w.writerows(rows)
    return len(rows)
...
def main(argv=None) -> int:
    ...
    print(f"  wrote cdifxas-units.tsv             ({n_units} concepts)")
