...
defines *what a thing of that kind can have*. A concept absent from
`NXxas` is therefore not necessarily absent from NeXus.

A first pass of this analysis mistakenly compared the glossary against
`NXxas` alone and concluded that ~15 beamline/facility concepts had no
NeXus home. **That was wrong.** Checking the base classes shows most of
them are well covered. The corrected picture is below.

---

## 1. Where the XAS-fork NXxas is heading

The fork **restructured** NXxas rather than editing it. `applications/NXxas.nxdl.xml`
...
## 3. Beamline / facility concepts — **corrected**

These were initially reported as gaps. Checking base classes, most are
covered:
...
CDIF glossary under a separate `xdi:` namespace** to make explicit that
they originate from the XDI specification rather than from NeXus, and
that no NeXus crosswalk target exists.
...
4. **Move the genuinely unhomed XDI-derived concepts to an `xdi:`
   namespace** (§3) so their provenance is explicit.
